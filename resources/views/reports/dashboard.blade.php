...
<div class="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-fade-in-up">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between bg-gradient-to-br from-[#1e1e2d] to-[#151521] border border-gray-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h2 class="text-3xl font-black text-white flex items-center gap-3">
                <i data-lucide="bar-chart-2" class="w-8 h-8 text-red-500"></i> التقارير والإحصائيات
            </h2>
            <p class="text-gray-400 mt-2 text-lg font-medium">نظرة شاملة على أداء النظام وفريق الدعم الفني</p>
        </div>
        <div class="mt-4 md:mt-0 relative z-10">
            <button class="bg-gray-800 border border-gray-700 text-gray-300 px-6 py-3 rounded-xl font-bold hover:bg-gray-700 hover:text-white transition-colors flex items-center gap-2">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        @foreach([
            ...
            ['تذاكر مفتوحة', $open, 'from-orange-600/20 to-orange-500/5', 'text-orange-400', 'border-orange-500/30', 'shadow-[0_0_15px_rgba(249,115,22,0.15)]'],
            ...
            ['معدل الحل', $resolveRate . '%', 'from-red-600/20 to-red-500/5', 'text-red-400', 'border-red-500/30', 'shadow-[0_0_15px_rgba(239,68,68,0.15)]'],
        ...
        <div class="bg-gradient-to-br {{ $bgGradient }} rounded-3xl border {{ $borderColor }} p-6 {{ $shadow }} relative overflow-hidden group hover:-translate-y-1 transition-transform">
            ...
            <div class="relative z-10">
                ...
                <p class="text-sm font-bold text-gray-400 mt-2 tracking-wide uppercase">{{ $label }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-8">
        <!-- By Type -->
        <div class="bg-[#1a1a24] rounded-3xl border border-gray-800 p-8 shadow-lg relative overflow-hidden">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex justify-between text-sm font-bold mb-2">
                        <span class="text-gray-300">{{ $typeLabels[$item->type] ?? $item->type }}</span>
                    ...
                    <div class="w-full bg-gray-900 rounded-full h-2.5 overflow-hidden border border-gray-800">
                        <div class="bg-gradient-to-r from-red-600 to-orange-500 h-full rounded-full relative" style="width: {{ $total > 0 ? round($item->count/$total*100) : 0 }}%">
        ...
        <div class="bg-[#1a1a24] rounded-3xl border border-gray-800 p-8 shadow-lg relative overflow-hidden">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex justify-between text-sm font-bold mb-2">
                        <span class="text-gray-300">{{ $priorityLabels[$item->priority] ?? $item->priority }}</span>
                        <span class="text-white">{{ $item->count }}</span>
                    </div>
                    <div class="w-full bg-gray-900 rounded-full h-2.5 overflow-hidden border border-gray-800">
    ...
    <!-- Team Performance -->
    <div class="bg-[#1a1a24] rounded-3xl border border-gray-800 overflow-hidden shadow-lg">
        <div class="p-8 border-b border-gray-800 bg-gray-900/30">
            <h3 class="text-lg font-bold text-white flex items-center gap-2">
                <i data-lucide="award" class="w-5 h-5 text-red-500"></i> أداء فريق الدعم
            </h3>
        </div>
            ...
            <table class="w-full text-right border-collapse">
                ...
                <tbody class="divide-y divide-gray-800/50">
                    ...
                    <tr class="hover:bg-gray-800/40 transition-colors">
                        <td class="px-8 py-5 text-sm font-bold text-gray-200 flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-xs text-gray-400">
                                {{ mb_substr($member->name, 0, 1) }}
                            </div>
                            {{ $member->name }}
                        </td>
                        <td class="px-8 py-5 text-center text-gray-400 font-bold">{{ $member->total_assigned }}</td>
                        <td class="px-8 py-5 text-center text-emerald-400 font-bold">{{ $member->resolved_count }}</td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div class="flex-1 bg-gray-900 rounded-full h-2 overflow-hidden border border-gray-800">
